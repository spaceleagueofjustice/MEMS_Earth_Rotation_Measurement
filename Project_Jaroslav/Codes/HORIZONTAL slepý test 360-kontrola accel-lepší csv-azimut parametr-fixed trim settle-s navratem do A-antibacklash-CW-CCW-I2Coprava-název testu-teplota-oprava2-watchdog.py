...
# CELE MERENI NAHODNE - POZICE I SMER (bez umeleho vynucovani smeru)
# + TEPLOTA Z ADXRS290 (TEMP0/TEMP1) + WARM-UP GATE PRED STARTEM
# + WATCHDOG NA ZASEKNUTOU SPI KOMUNIKACI (NOVINKA)
#
...
# i do logu.
#
# NEMA17 + A4988 + ADXRS290 (SPI1) + MPU6050 (I2C0) - kontrola naklonu
#
# FILOZOFIE TETO VERZE (po diskuzi - zadne umele "laboratorni"
# vynucovani smeru):
#   Kazdy cyklus se vybere NAHODNY cilovy azimut (z cele mnoziny 360
#   stupnu) a motor k nemu vzdy jede NEJKRATSI cestou (presne jako od
#   zacatku - bezpecne, levne, beze zmeny puvodni, jiz overene
#   mechaniky). Smer (CW/CCW), kterym motor k cili dojede, se proste
#   ZAZNAMENA (podle znamenka najete odchylky), NEVYNUCUJE se.
#
#   Predtim zkousene "vynuceni smeru" (aby sel vybrat CW i CCW
#   nezavisle na tom, kam se ma realne jet) vyzadovalo casto jet
#   "oklikou" pres skoro cely kruh navic - v kombinaci s tvrdym
#   kabelovym limitem 400 stupnu by to znamenalo odvijeni kabelu u
#   ~85 % cyklu. Nepraktcke a zbytecne "laboratorni".
#
#   Protoze je cilovy azimut kazdy cyklus nahodny, vychazi smer
#   najeti prirozene cca 50/50 CW/CCW (overeno simulaci na 5000
#   cyklech: 50.3 % / 49.7 %) - staci ho jen zapsat do CSV, nic
#   vynucovat netreba. Kabel se odviji stejne vzacne jako drive
#   (~6 % cyklu, overeno simulaci).
#
# NOVINKA - WATCHDOG NA ZASEKNUTOU SPI KOMUNIKACI:
#   Objevilo se, ze ADXRS290 muze uprostred dlouheho behu "zaseknout" -
#   SPI transakce probihaji bez chyby (zadny OSError, retry logika to
#   tedy nezachyti), ale gX/gY/teplota zustanou bit-presne konstantni
#   po zbytek behu (tiche selhani). Watchdog nize hlida poslednich
#   STUCK_CHECK_COUNT cteni; pokud jsou identicka, povazuje se to za
#   zaseknuti (NE platny signal) a zkusi se softwarova reinicializace
#   SPI periferie + registru cipu (metoda ADXRS290.reinit()).
#   DULEZITE: reinit() se NIKDY nedotyka motoru (EN/DIR/STEP pinu) -
#   je to cistě SPI/cip operace, rameno se timhle nemuze pootocit.
#
# Postup: 1) rucne nastavit rameno na sever (jednou, na zacatku).
#         2) spustit skript.
#         3) skript pocka na ustaleni teploty gyra (warm-up gate).
#         4) skript automaticky odjizdi TOTAL_CYCLES nahodnych
#            pozic, smer kazdeho najeti se jen zaznamena do CSV.
...
AZIMUTH_STEP_DEG = 10 #22.5
AZIMUTHS         = [round(i * AZIMUTH_STEP_DEG, 2) for i in range(int(round(360 / AZIMUTH_STEP_DEG)))]

# Celkovy pocet mericich cyklu (kazdy = 1 nahodne zvoleny azimut,
# zmereny jako pár A/C). Puvodne bylo 15 opakovani/azimut x 16
# azimutu x 2 smery = 480 mereni celkem - TOTAL_CYCLES=480 dava
# srovnatelny celkovy cas behu.
TOTAL_CYCLES         = 480 # 480
SAMPLES_PER_POSITION = 200
SAMPLE_DELAY         = 0.01

FIXED_SETTLE_S = 4.0    # plne usazeni po KAZDEM finalnim priblizeni (pred merenim)
SETTLE_PRE_S   = 0.5    # kratke usazeni po hrubem presunu, pred finalnim priblizenim
FIXED_TRIM     = 50

CABLE_SAFETY_LIMIT_DEG = 400.0   # TVRDY limit - nikdy se neprekroci
UNWIND_WAIT_S          = 1.0

# --- Anti-backlash: jednosmerne najizdeni na KAZDY cil (nemenit) ---
...
WARMUP_TIMEOUT_S                 = 900.0   # PARAMETR: max. cekani (15 min), pak se stejne spusti mereni
...
# =============================================================================
# ANTI-BACKLASH POHYB (absolutni, nezabalovana pozice) - NEJKRATSI CESTA
# =============================================================================

_current_pos = 0.0

def move_to(target_pos, settle_after=None):
    """Presun na absolutni cil VZDY nejkratsi (primou) cestou, s
    anti-backlash presahem pred finalnim najetim. Zadne vynucovani
    smeru - smer vyjde prirozene ze znamenka delty."""
    global _current_pos
...
def move_to_azimuth(target_azimuth):
    """Presun na novy cilovy azimut nejkratsi cestou, s anti-backlash
    najizdenim a tvrdou ochranou kabelaze (nikdy nad CABLE_SAFETY_LIMIT_DEG).
    Vraci (diff, smer_label) - smer se JEN ZAZNAMENA podle znamenka
    najete odchylky, nevynucuje se."""
    global _current_pos
    ...
    if abs(target_abs) > CABLE_SAFETY_LIMIT_DEG:
        ...
        log(f"  [KABEL] Kumulativni natoceni {_current_pos:.1f} deg by presahlo limit "
            f"{CABLE_SAFETY_LIMIT_DEG:.0f} deg. Odvijim zpet na sever ({unwind:+.1f} deg)...")
    ...
    log(f"  Presun na azimut {target_azimuth} deg (delta={diff:+.1f} deg, smer={smer_label}, "
        f"pozice pred presunem={pos_before:+.1f} deg, anti-backlash aktivni)")
...
# Viz komentar v hlavicce souboru. Cistě softwarova operace na SPI/cipu -
# motor (EN/DIR/STEP piny) se timhle vubec nedotkne.
# =============================================================================
...
def csv_header_once():
    try:
        ...
        with open(CSV_FILE, "w") as f:
            f.write("smer;cyklus;cilovy_azimut;skutecne_natoceni;jump_deg;trim;"
                     "xA;yA;tA;axA;ayA;xC;yC;tC;axC;ayC\n")

# =============================================================================
# JEDEN MERICI CYKLUS - nahodny azimut, smer se zaznamena (nevynucuje)
# =============================================================================

def run_single_cycle(gyro, mpu, cycle_num, total_cycles, azimuth_stats):
    target_az = AZIMUTHS[random.randint(0, len(AZIMUTHS) - 1)]

    log("\n" + "-"*65)
    log(f"CYKLUS {cycle_num}/{total_cycles}  ->  nahodny cilovy azimut {target_az} deg")
    log("-"*65)

    success = False
    for cycle_attempt in range(1, 4):
        try:
            diff, smer_label = move_to_azimuth(target_az)
            actual_az = _current_pos
            ...
            log("\nPresouvam C->A (navrat, anti-backlash)...")
            move_relative(-180)

            with open(CSV_FILE, "a") as f:
                f.write(f"{smer_label};{cycle_num};{target_az:g};{actual_az:.2f};{diff:.2f};{FIXED_TRIM};"
                        f"{A_X:.8f};{A_Y:.8f};{A_T:.2f};{A_AX:.6f};{A_AY:.6f};"
                        f"{C_X:.8f};{C_Y:.8f};{C_T:.2f};{C_AX:.6f};{C_AY:.6f}\n")

            key = (smer_label, target_az)
            if key not in azimuth_stats:
                azimuth_stats[key] = {
            ...
            s['yA']  += A_Y;  s['yC']  += C_Y
            s['tA']  += A_T;  s['tC']  += C_T
            s['axA'] += A_AX; s['axC'] += C_AX
            s['ayA'] += A_AY; s['ayC'] += C_AY

            log(f"\n  Cyklus {cycle_num} ({smer_label}, {target_az} deg) zapsan do CSV.")
            success = True
            break
        except OSError as e:
            log(f"  [CHYBA] Cyklus {cycle_num} (azimut {target_az} deg) selhal na pokusu "
                f"{cycle_attempt}/3: {e}")
            time.sleep(1)

    if not success:
        log(f"  [CHYBA] Cyklus {cycle_num} (azimut {target_az} deg) TRVALE selhal po 3 pokusech - "
            f"PRESKAKUJI a pokracuji dalsim cyklem.")

def write_summary(stats):
    with open(SUMMARY_FILE, "w") as f:
        f.write("smer;azimut;n;xA_prumer;xC_prumer;diffx_prumer;"
        ...
        for (smer_label, az) in sorted(stats.keys(), key=lambda k: (k[0], k[1])):
            s = stats[(smer_label, az)]
            n = s['n']
            if n == 0:
                continue
            xA_m  = s['xA']  / n;  xC_m  = s['xC']  / n
            yA_m  = s['yA']  / n;  yC_m  = s['yC']  / n
            tA_m  = s['tA']  / n;  tC_m  = s['tC']  / n
            axA_m = s['axA'] / n;  axC_m = s['axC'] / n
            ayA_m = s['ayA'] / n;  ayC_m = s['ayC'] / n
            f.write(f"{smer_label};{az:g};{n};{xA_m:.8f};{xC_m:.8f};{xA_m - xC_m:.8f};"
...
def main():
    with open(LOG_FILE, "w") as f:
        f.write(f"=== EARTH ROTATION LOG - TESTID={TESTID} (nahodna pozice i smer) ===\n")
    ...
    log("NAHODNA POZICE I SMER (smer se zaznamenava, nevynucuje)")
    log("NEMA17 + A4988 + ADXRS290 (+teplota) + MPU6050 (kontrola naklonu)")
    log("="*65)
    log("\nPOZOR: pozice A (azimut 0 deg) musi byt pred spustenim nastavena")
    log("RUCNE (osa X miri na sever), sestava vyrovnana a od ted uz s ni")
    log("NEHYBAT. Vsechny dalsi presuny dela vyhradne motor, vzdy nejkratsi")
    log("cestou. Motor zustava zapnuty po celou dobu behu.")
    ...
    log("\n" + "="*65)
    log(f"CELKEM {TOTAL_CYCLES} nahodnych cyklu (pozice i smer nahodne, smer jen zaznamenan)")
    log(f"AZIMUTH_STEP_DEG = {AZIMUTH_STEP_DEG}  ({len(AZIMUTHS)} moznych azimutu)")
    ...
    log(f"CABLE_SAFETY_LIMIT_DEG = {CABLE_SAFETY_LIMIT_DEG:.0f} (tvrdy limit)")
    log("="*65)

    for cycle_num in range(1, TOTAL_CYCLES + 1):
        run_single_cycle(gyro, mpu, cycle_num, TOTAL_CYCLES, azimuth_stats)
        if cycle_num % 20 == 0:
            beep(count=1, freq=2000, duration_ms=100)
            log(f"\n[POSTUP] Dokonceno {cycle_num}/{TOTAL_CYCLES} cyklu.")

    write_summary(azimuth_stats)
